...
contract ZyroToken is ITokenStandard, ITokenDetails {
    ...
    string private constant TOKEN_NAME = "Zyro";
    string private constant TOKEN_SYMBOL = "ZYRO";
    ...
    uint256 private constant STARTING_SUPPLY = 90e9 * 1e18; // 90 billion tokens
    ...
    function sendTokens(address recipientWallet, uint256 amount) external onlyAdmin nonReentrant {
        ...
        uint256 adminBalance = tokenBalances[msg.sender];
    ...
    function _executeTransfer(address from, address to, uint256 amount) internal {
        ...
        require(amount <= senderBalance, "ZyroToken: insufficient balance");
    ...
    function _destroyTokens(address account, uint256 amount) internal {
        ...
        require(amount <= accountBalance, "ZyroToken: burn exceeds balance");
        tokenBalances[account] -= amount;
        totalTokenSupply -= amount;
    ...
    function _deductApproval(address owner, address spender, uint256 amount) internal {
        uint256 currentApproval = approvedTokens[owner][spender];
        if (currentApproval != type(uint256).max) {
            require(amount <= currentApproval, "ZyroToken: insufficient approval");
            approvedTokens[owner][spender] -= amount;
            emit TokenApproved(owner, spender, approvedTokens[owner][spender]);
        }
    }
}
