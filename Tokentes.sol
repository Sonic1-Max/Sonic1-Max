...
contract TokenCore is BaseContext, ReentrancyGuard, ITokenStandard, ITokenDetails {
    // State Variables
    ...
    function moveTokens(address to, uint256 amount) public virtual override nonReentrant returns (bool) {
        require(amount > 0, "TokenCore: amount must be greater than zero");
    ...
    function approveSpender(address spender, uint256 amount) public virtual override returns (bool) {
        require(amount >= 0, "TokenCore: amount cannot be negative");
    ...
    function _executeTransfer(address from, address to, uint256 amount) internal virtual {
        ...
        require(tokenBalances[from] >= amount, "TokenCore: insufficient balance");
        unchecked {
    ...
    function _destroyTokens(address account, uint256 amount) internal virtual {
        ...
        require(tokenBalances[account] >= amount, "TokenCore: burn exceeds balance");
        unchecked {
            tokenBalances[account] -= amount;
            totalTokenSupply -= amount;
        }
        
        // Tidak emit TokenMoved untuk burn, cukup event di ZyroToken
...
contract ZyroToken is TokenCore, AdminControl {
    ...
    // Admin Functions
    function sendTokens(address recipientWallet) external onlyAdmin nonReentrant {
